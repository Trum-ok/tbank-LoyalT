...
> TODO: записать и смонтировать демо-ролик (1.5–3 мин: партнёр собирает программу →
> клиент подключается по QR → касса начисляет баллы → push → дашборд аналитики),
> затем перетащить `.mp4` сюда через веб-редактор GitHub — подставится нативный плеер.
