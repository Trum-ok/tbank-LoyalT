"""Kafka consumer для входящих событий core.

Подписан на топики из `settings.kafka_subscribe_topics` (по умолчанию —
`partner.events`). Обрабатывает каждое сообщение в своей сессии и
коммитит оффсет только после успеха.

При `kafka_enabled=False` consumer не стартует; события можно
...
class EventConsumer:
    ...
    async def start(self) -> None:
        ...
        self._consumer = AIOKafkaConsumer(
            *self._settings.kafka_subscribe_topics,
        ...
        self._task = asyncio.create_task(self._run(), name="core-kafka-consumer")
        logger.info(
    ...
    async def _process_message(self, msg) -> None:  # type: ignore[no-untyped-def]
        ...
        async with SessionLocal() as session:
            try:
                ...
                logger.exception(
                    "Failed to process event type=%s offset=%s",
                    event_type,
                    msg.offset,
                )
                return
