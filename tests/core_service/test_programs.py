"""
Тесты новых настроек программ лояльности и уровней (тиров).

Структура:
  TestProgramSchemas  — Pydantic-валидация без обращения к БД
  TestProgramSettings — сервисный слой: создание/обновление расширенных полей
  TestProgramTiers    — CRUD уровней лояльности
"""
...
_VISIT_BASE = dict(type=ProgramType.VISIT, accrual_rule={"points_per_visit": 50})
_STAMPS_BASE = dict(type=ProgramType.STAMPS, accrual_rule={"visits_required": 8})
...
# ===========================================================================
# TestProgramSchemas — чистая валидация Pydantic
# ===========================================================================
...
# ===========================================================================
# TestProgramSettings — сервисный слой, новые поля Program
# ===========================================================================
...
class TestProgramSettings:
    async def test_create_with_welcome_bonus(
        ...
        data = _program(welcome_bonus_points=300)
        p = await service.create_program(session, partner_id, data)
        assert p.welcome_bonus_points == 300

    async def test_create_with_birthday_bonus(
        self, session: AsyncSession, partner_id: UUID
    ):
        data = _program(birthday_bonus_points=75, birthday_bonus_days=3)
        p = await service.create_program(session, partner_id, data)
    ...
    async def test_create_with_referral_bonus(
        ...
        data = _program(referral_bonus_points=150)
        p = await service.create_program(session, partner_id, data)
    ...
    async def test_create_with_purchase_restrictions(
        ...
        data = _program(min_purchase_amount=50000, max_points_per_transaction=1000)
        p = await service.create_program(session, partner_id, data)
    ...
    async def test_create_with_redemption_limit(
        ...
        data = _program(max_redemption_percent=30)
        p = await service.create_program(session, partner_id, data)
    ...
    async def test_create_with_validity_period(
        ...
        data = _program(valid_from=today, valid_until=end)
        p = await service.create_program(session, partner_id, data)
        assert p.valid_from == today
    ...
    async def test_update_clears_birthday_bonus(
        ...
        data = _program(birthday_bonus_points=100)
        p = await service.create_program(session, partner_id, data)
...
class TestProgramTiers:
    async def _make_program(self, session: AsyncSession, partner_id: UUID):
        return await service.create_program(session, partner_id, _program())

    async def test_add_tier_returns_updated_program(
        self, session: AsyncSession, partner_id: UUID
    ):
        p = await self._make_program(session, partner_id)
        result = await service.add_tier(
            session, p.id, partner_id, TierCreate(name="Бронзовый", threshold_points=0)
        )
    ...
    async def test_add_multiple_tiers_sorted_by_threshold(
        ...
        await service.add_tier(
            session, p.id, partner_id, TierCreate(name="Золотой", threshold_points=5000)
        )
        await service.add_tier(
            session, p.id, partner_id, TierCreate(name="Бронзовый", threshold_points=0)
        )
    ...
    async def test_add_tier_with_multiplier(
        self, session: AsyncSession, partner_id: UUID
    ):
        p = await self._make_program(session, partner_id)
        result = await service.add_tier(
    ...
    async def test_duplicate_name_raises(self, session: AsyncSession, partner_id: UUID):
        p = await self._make_program(session, partner_id)
        await service.add_tier(
            session, p.id, partner_id, TierCreate(name="Бронзовый", threshold_points=0)
        )
    ...
    async def test_duplicate_threshold_raises(
        self, session: AsyncSession, partner_id: UUID
    ):
        p = await self._make_program(session, partner_id)
        await service.add_tier(
    ...
    async def test_update_tier_name(self, session: AsyncSession, partner_id: UUID):
        p = await self._make_program(session, partner_id)
        result = await service.add_tier(
            session, p.id, partner_id, TierCreate(name="Bronze", threshold_points=0)
        )
        tier_id = result.tiers[0].id

        updated = await service.update_tier(
            session, p.id, tier_id, partner_id, TierUpdate(name="Бронзовый")
        )
        assert updated.tiers[0].name == "Бронзовый"

    async def test_update_tier_multiplier(self, session: AsyncSession, partner_id: UUID):
        p = await self._make_program(session, partner_id)
        result = await service.add_tier(
            session, p.id, partner_id, TierCreate(name="VIP", threshold_points=5000)
        )
        tier_id = result.tiers[0].id
    ...
    async def test_delete_tier(self, session: AsyncSession, partner_id: UUID):
        p = await self._make_program(session, partner_id)
        result = await service.add_tier(
            session, p.id, partner_id, TierCreate(name="Bronze", threshold_points=0)
        )
        ...
        final = await service.delete_tier(session, p.id, tier_id, partner_id)
        assert len(final.tiers) == 0

    async def test_delete_one_of_multiple_tiers(
        self, session: AsyncSession, partner_id: UUID
    ):
        p = await self._make_program(session, partner_id)
        ...
        final = await service.delete_tier(session, p.id, bronze_id, partner_id)
        assert len(final.tiers) == 1
        assert final.tiers[0].name == "Серебряный"

    async def test_archived_program_cannot_add_tier(
        self, session: AsyncSession, partner_id: UUID
    ):
        p = await self._make_program(session, partner_id)
        await service.transition_status(
            session, p.id, partner_id, ProgramStatus.ARCHIVED
        )
    ...
    async def test_archived_program_cannot_update_tier(
        ...
        with pytest.raises(BadRequestError, match="archived"):
            await service.update_tier(
                session, p.id, tier_id, partner_id, TierUpdate(name="Gold")
            )
    ...
    async def test_archived_program_cannot_delete_tier(
        ...
        with pytest.raises(BadRequestError, match="archived"):
            await service.delete_tier(session, p.id, tier_id, partner_id)

    async def test_tier_not_found_raises(self, session: AsyncSession, partner_id: UUID):
        p = await self._make_program(session, partner_id)
        with pytest.raises(NotFoundError):
            await service.delete_tier(session, p.id, uuid4(), partner_id)

    async def test_tier_belongs_to_another_program_raises(
        self, session: AsyncSession, partner_id: UUID
    ):
        p1 = await self._make_program(session, partner_id)
    ...
    async def test_new_program_has_empty_tiers(
        self, session: AsyncSession, partner_id: UUID
    ):
        p = await self._make_program(session, partner_id)
        assert p.tiers == []

    async def test_tier_ids_are_unique(self, session: AsyncSession, partner_id: UUID):
        p = await self._make_program(session, partner_id)
        await service.add_tier(
            session, p.id, partner_id, TierCreate(name="Бронзовый", threshold_points=0)
        )
